(() => {
  ...
  function renderStructural(rows, data) {
    ...
    if (data.clearing.length) warnings.push('Campos operacionais presentes e vazios serão limpos no banco.');

    panel.hidden = false;
    panel.innerHTML = `
      <div class="upload-preview-head">
        <div><span>Prévia da importação</span><h3>Impacto estrutural e comparação com o banco</h3></div>
      ...
      <div class="upload-preview-metrics upload-preview-structural">
        ...
        ${metric('Campos operacionais', data.operational.length, `${data.clearing.length} com células vazias`)}
      </div>
      ...
      ${warnings.length ? `<div class="upload-preview-warnings">${warnings.map(warning => `<p>⚠ ${warning}</p>`).join('')}</div>` : '<div class="upload-preview-ok">Arquivo estruturalmente pronto para importação.</div>'}
    `;
  }
  ...
  function renderDatabasePreview(data) {
    ...
    target.innerHTML = `
      ...
      <div class="upload-preview-metrics upload-preview-db-metrics">
        ${metric('Novos', data.novos || 0)}
        ${metric('Existentes', data.existentes || 0)}
        ${metric('Serão alterados', data.alterados || 0)}
        ${metric('Sem mudança', data.sem_mudanca || 0)}
        ...
        ${metric('Limpezas previstas', data.limpezas || 0, 'Campos operacionais')}
      </div>
      ${(data.ambiguos || data.rejeitados) ? '<div class="upload-preview-warnings"><p>⚠ Revise os registros ambíguos ou rejeitados antes da importação.</p></div>' : '<div class="upload-preview-ok">Nenhum conflito de identificação encontrado na simulação.</div>'}
    `;
  }
  ...
  document.addEventListener('change', async event => {
    ...
    try {
      ...
      const data = structuralData(rows);
      renderStructural(rows, data);
      try {
        renderDatabasePreview(await fetchDatabasePreview(rows, data.headers));
      } catch (error) {
        const target = document.querySelector('#uploadDbPreview');
  ...
  style.textContent = `
    ...
    .upload-db-preview{margin-top:14px;padding-top:14px;border-top:1px solid rgba(148,163,184,.16)}.upload-preview-db-metrics{grid-template-columns:repeat(7,minmax(0,1fr))}
    ...
    @media(max-width:1200px){.upload-preview-db-metrics{grid-template-columns:repeat(4,minmax(0,1fr))}}@media(max-width:800px){.upload-preview-metrics,.upload-preview-db-metrics{grid-template-columns:1fr 1fr}.upload-preview-columns{grid-template-columns:1fr}}
  `;
