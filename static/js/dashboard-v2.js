(() => {
  ...
  async function loadEducationKpis() {
    ...
    try {
      ...
      number('#kpiQueue', d.fila_disparo); number('#kpiToday', d.inscritos_hoje);
      number('#kpi7d', d.inscritos_7_dias); number('#kpiDispatchedToday', d.disparados_hoje);
      number('#kpiEnrolled', d.matriculas); number('#kpiBacklog', d.backlog_3_dias);
  ...
  function scheduleKpis() { clearTimeout(kpiTimer); kpiTimer = setTimeout(loadEducationKpis, 320); }
  ...
  function enhanceRows() {
    ...
    rows.forEach((row) => {
      ...
      row.classList.toggle('ops-ready-row', isReady); row.classList.toggle('ops-sent-row', !isReady);
      if (isReady) ready += 1;
    });
  ...
  function setQueueFilter(mode) {
    const situation = $('#fDataDisparoSituacao'); const ini = $('#fIni'); const fim = $('#fFim');
    document.querySelectorAll('.quick-action').forEach((b) => b.classList.remove('active'));
    if (mode === 'queue') {
      if (situation) situation.value = 'vazias'; if (ini) ini.value = ''; if (fim) fim.value = '';
      $('#quickQueue')?.classList.add('active');
    } else if (mode === 'today') {
      const today = new Date().toISOString().slice(0,10);
      if (situation) situation.value = ''; if (ini) ini.value = today; if (fim) fim.value = today;
      $('#quickToday')?.classList.add('active');
    } else {
      if (situation) situation.value = ''; if (ini) ini.value = ''; if (fim) fim.value = '';
  ...
  function init() {
    ...
    $('#toggleFilters')?.addEventListener('click', () => {
      const panel = $('#filterPanel'); const toggle = $('#toggleFilters');
