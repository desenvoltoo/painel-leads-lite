...
<title>Falha ao inicializar aplicação</title><style>
body {{ font-family: Arial,sans-serif; margin:32px; background:#fff7ed; color:#1f2937; }}
main {{ max-width:980px; margin:auto; background:white; border:1px solid #fed7aa; border-radius:12px; padding:24px; }}
h1 {{ color:#c2410c; }} pre {{ background:#111827; color:#f9fafb; padding:16px; border-radius:8px; overflow:auto; white-space:pre-wrap; }}
</style></head><body><main><h1>Falha ao inicializar aplicação</h1><pre>{escaped_json}</pre></main></body></html>"""
