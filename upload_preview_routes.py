# -*- coding: utf-8 -*-
"""Rota de prévia de importação usando exatamente a mesma view do painel."""
        ...
        try:
            ...
            if not rows:
                return jsonify({"ok": True, "data": {
                    ...
                    "fonte_comparacao": db._view_table_id(),
                    "ordem_identificacao": ["celular", "cpf"],
                }})

            # Usa exatamente a mesma view configurada para a listagem do painel.
            ...
            telefone_match AS (
                SELECT
                    e.linha,
                    MIN(v.sk_pessoa::text) AS sk_pessoa,
                    COUNT(*)::bigint AS qtd
                FROM entrada e
                JOIN {view_id} v
                  ON e.celular IS NOT NULL
                 AND regexp_replace(COALESCE(v.celular::text, ''), '[^0-9]', '', 'g') = e.celular
                GROUP BY e.linha
            ),
            cpf_match AS (
                SELECT
                ...
                JOIN {view_id} v
                 ...
                 AND regexp_replace(COALESCE(v.cpf::text, ''), '[^0-9]', '', 'g') = e.cpf
                GROUP BY e.linha
            ),
            resolvida AS (
                SELECT
                    e.*,
                    CASE
                        WHEN COALESCE(tm.qtd, 0) = 1 THEN tm.sk_pessoa
                        WHEN COALESCE(tm.qtd, 0) = 0 AND COALESCE(cm.qtd, 0) = 1 THEN cm.sk_pessoa
                        ELSE NULL
                    END AS sk_pessoa_resolvida,
                    COALESCE(tm.qtd, 0) AS celular_matches,
                    COALESCE(cm.qtd, 0) AS cpf_matches,
                    CASE
                        WHEN COALESCE(tm.qtd, 0) = 1 THEN 'CELULAR'
                        WHEN COALESCE(tm.qtd, 0) = 0 AND COALESCE(cm.qtd, 0) = 1 THEN 'CPF'
                        ELSE NULL
                    END AS encontrado_por
                FROM entrada e
                LEFT JOIN telefone_match tm ON tm.linha = e.linha
                LEFT JOIN cpf_match cm ON cm.linha = e.linha
            )
            SELECT
                r.*,
                v.consultor_comercial AS atual_consultor_comercial,
                v.consultor_disparo AS atual_consultor_disparo,
                v.status AS atual_status,
                v.status_inscricao AS atual_status_inscricao,
                v.campanha AS atual_campanha,
                v.canal AS atual_canal,
                v.acao_comercial AS atual_acao_comercial,
                v.tipo_disparo AS atual_tipo_disparo,
                v.peca_disparo AS atual_peca_disparo,
                v.texto_disparo AS atual_texto_disparo,
                v.observacao AS atual_observacao,
                v.qtd_acionamentos::text AS atual_qtd_acionamentos,
                v.flag_matriculado::text AS atual_matriculado,
                v.data_ultima_acao::text AS atual_data_ultima_acao,
                v.data_disparo::text AS atual_data_disparo
            FROM resolvida r
            LEFT JOIN {view_id} v
              ON v.sk_pessoa::text = r.sk_pessoa_resolvida
            ORDER BY r.linha
            """
            result = db._run_gestao_query(
                sql,
                {"rows_json": json.dumps(rows, ensure_ascii=False, default=str)},
                "upload_preview_real_same_panel_view",
            )
            ...
            metrics = {
                ...
                "encontrados_por_celular": 0, "encontrados_por_cpf": 0,
            }
            ...
            for item in result:
                ...
                sk = item.get("sk_pessoa_resolvida")
                ...
                # Celular tem prioridade. CPF só é avaliado quando celular não encontrou.
                if celular_matches > 1 or (celular_matches == 0 and cpf_matches > 1):
                    metrics["ambiguos"] += 1
                ...
                if item.get("encontrado_por") == "CELULAR":
                    metrics["encontrados_por_celular"] += 1
                elif item.get("encontrado_por") == "CPF":
                    metrics["encontrados_por_cpf"] += 1
