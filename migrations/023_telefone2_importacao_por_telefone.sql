-- Adiciona telefone secundario e torna a triagem de "somente novos"
-- prioritariamente baseada em telefone.
--
-- Regras:
-- 1. telefone ausente: rejeita;
-- 2. telefone existente em celular ou telefone2: ignora;
-- 3. telefone novo + CPF existente + telefone2 vazio: grava telefone2;
-- 4. telefone novo + CPF existente + telefone2 ocupado: registra conflito;
-- 5. telefone e CPF novos: envia para a procedure principal.

BEGIN;

ALTER TABLE modelo_estrela.dim_pessoa
    ADD COLUMN IF NOT EXISTS telefone2 text;

ALTER TABLE unifecaf.dim_pessoa
    ADD COLUMN IF NOT EXISTS telefone2 text;

COMMENT ON COLUMN modelo_estrela.dim_pessoa.telefone2 IS
    'Telefone secundario da pessoa, normalizado pela rotina de importacao.';

COMMENT ON COLUMN unifecaf.dim_pessoa.telefone2 IS
    'Telefone secundario da pessoa, normalizado pela rotina de importacao.';

CREATE INDEX IF NOT EXISTS idx_modelo_dim_pessoa_telefone2_normalizado
ON modelo_estrela.dim_pessoa (
    regexp_replace(COALESCE(telefone2, ''), '[^0-9]', '', 'g')
);

CREATE INDEX IF NOT EXISTS idx_unifecaf_dim_pessoa_telefone2_normalizado
ON unifecaf.dim_pessoa (
    regexp_replace(COALESCE(telefone2, ''), '[^0-9]', '', 'g')
);

CREATE OR REPLACE FUNCTION modelo_estrela.sp_importar_somente_leads_novos(
    p_upload_id text
)
RETURNS TABLE(
    upload_id text,
    linhas_recebidas bigint,
    linhas_novas bigint,
    existentes_por_celular bigint,
    existentes_por_cpf bigint,
    duplicados_no_arquivo bigint,
    linhas_sem_identificador bigint,
    linhas_enviadas_procedure bigint,
    linhas_inseridas bigint,
    linhas_rejeitadas bigint,
    status text,
    mensagem text
)
LANGUAGE plpgsql
AS $function$
DECLARE
    v_total bigint := 0;
    v_novos bigint := 0;
    v_existentes_telefone bigint := 0;
    v_cpf_telefone2 bigint := 0;
    v_conflitos_telefone2 bigint := 0;
    v_duplicados bigint := 0;
    v_sem_telefone bigint := 0;
    v_inseridas bigint := 0;
    v_rejeitadas bigint := 0;
    v_status text := 'CONCLUIDO';
    v_mensagem text := '';
BEGIN
    IF NULLIF(BTRIM(p_upload_id), '') IS NULL THEN
        RAISE EXCEPTION 'upload_id e obrigatorio';
    END IF;

    PERFORM pg_advisory_xact_lock(
        hashtext('SP_IMPORTAR_SOMENTE_NOVOS:' || p_upload_id)
    );

    SELECT COUNT(*)
      INTO v_total
      FROM modelo_estrela.stg_leads_site s
     WHERE s.upload_id = p_upload_id;

    IF v_total = 0 THEN
        RAISE EXCEPTION 'Nenhuma linha encontrada para o upload_id %', p_upload_id;
    END IF;

    IF v_total > 15000 THEN
        RAISE EXCEPTION 'O upload % possui % linhas. O limite e 15.000.', p_upload_id, v_total;
    END IF;

    DROP TABLE IF EXISTS pg_temp.tmp_modelo_telefones;
    DROP TABLE IF EXISTS pg_temp.tmp_modelo_triagem;

    CREATE TEMP TABLE tmp_modelo_telefones
    ON COMMIT DROP
    AS
    SELECT sk_pessoa, telefone_limpo
    FROM (
        SELECT
            p.sk_pessoa,
            modelo_estrela.fn_somente_numeros(p.celular) AS telefone_limpo
        FROM modelo_estrela.dim_pessoa p

        UNION ALL

        SELECT
            p.sk_pessoa,
            modelo_estrela.fn_somente_numeros(p.telefone2) AS telefone_limpo
        FROM modelo_estrela.dim_pessoa p
    ) telefones
    WHERE telefone_limpo IS NOT NULL;

    CREATE INDEX ON tmp_modelo_telefones (telefone_limpo);
    ANALYZE tmp_modelo_telefones;

    CREATE TEMP TABLE tmp_modelo_triagem
    ON COMMIT DROP
    AS
    WITH normalizada AS (
        SELECT
            s.ctid AS row_ctid,
            s.linha_arquivo,
            modelo_estrela.fn_somente_numeros(s.celular) AS telefone_limpo,
            modelo_estrela.fn_somente_numeros(s.cpf) AS cpf_limpo
        FROM modelo_estrela.stg_leads_site s
        WHERE s.upload_id = p_upload_id
    ),
    classificada AS (
        SELECT
            n.*,
            ROW_NUMBER() OVER (
                PARTITION BY n.telefone_limpo
                ORDER BY n.linha_arquivo NULLS LAST, n.row_ctid
            ) AS rn_telefone,
            tp.sk_pessoa AS pessoa_por_telefone,
            pc.sk_pessoa AS pessoa_por_cpf,
            modelo_estrela.fn_somente_numeros(pc.celular) AS celular_atual,
            modelo_estrela.fn_somente_numeros(pc.telefone2) AS telefone2_atual
        FROM normalizada n
        LEFT JOIN LATERAL (
            SELECT t.sk_pessoa
            FROM tmp_modelo_telefones t
            WHERE t.telefone_limpo = n.telefone_limpo
            ORDER BY t.sk_pessoa
            LIMIT 1
        ) tp ON n.telefone_limpo IS NOT NULL
        LEFT JOIN LATERAL (
            SELECT p.sk_pessoa, p.celular, p.telefone2
            FROM modelo_estrela.dim_pessoa p
            WHERE n.cpf_limpo IS NOT NULL
              AND modelo_estrela.fn_somente_numeros(p.cpf) = n.cpf_limpo
            ORDER BY p.sk_pessoa
            LIMIT 1
        ) pc ON TRUE
    )
    SELECT
        c.*,
        CASE
            WHEN c.telefone_limpo IS NULL THEN 'SEM_TELEFONE'
            WHEN c.rn_telefone > 1 THEN 'DUPLICADO_ARQUIVO'
            WHEN c.pessoa_por_telefone IS NOT NULL THEN 'EXISTENTE_TELEFONE'
            WHEN c.pessoa_por_cpf IS NOT NULL
             AND (c.telefone2_atual IS NULL OR c.telefone2_atual = c.telefone_limpo)
                THEN 'CPF_ATUALIZAR_TELEFONE2'
            WHEN c.pessoa_por_cpf IS NOT NULL
                THEN 'CONFLITO_TELEFONE2'
            ELSE 'NOVO'
        END AS classificacao
    FROM classificada c;

    CREATE INDEX ON tmp_modelo_triagem (classificacao);
    ANALYZE tmp_modelo_triagem;

    UPDATE modelo_estrela.dim_pessoa p
       SET telefone2 = t.telefone_limpo,
           updated_at = now()
      FROM tmp_modelo_triagem t
     WHERE t.classificacao = 'CPF_ATUALIZAR_TELEFONE2'
       AND p.sk_pessoa = t.pessoa_por_cpf
       AND modelo_estrela.fn_somente_numeros(p.celular) IS DISTINCT FROM t.telefone_limpo
       AND modelo_estrela.fn_somente_numeros(p.telefone2) IS DISTINCT FROM t.telefone_limpo;

    INSERT INTO modelo_estrela.logs_rejeicoes_import (
        ts, motivo, cpf_raw, celular_raw, upload_id, linha, campo,
        valor_mascarado, criado_em
    )
    SELECT
        now(),
        CASE
            WHEN t.classificacao = 'SEM_TELEFONE' THEN 'SEM_TELEFONE'
            ELSE 'CONFLITO_TELEFONE2'
        END,
        s.cpf,
        s.celular,
        p_upload_id,
        s.linha_arquivo,
        'celular',
        NULL,
        now()
    FROM tmp_modelo_triagem t
    JOIN modelo_estrela.stg_leads_site s ON s.ctid = t.row_ctid
    WHERE t.classificacao IN ('SEM_TELEFONE', 'CONFLITO_TELEFONE2');

    SELECT
        COUNT(*) FILTER (WHERE classificacao = 'NOVO'),
        COUNT(*) FILTER (WHERE classificacao = 'EXISTENTE_TELEFONE'),
        COUNT(*) FILTER (WHERE classificacao = 'CPF_ATUALIZAR_TELEFONE2'),
        COUNT(*) FILTER (WHERE classificacao = 'CONFLITO_TELEFONE2'),
        COUNT(*) FILTER (WHERE classificacao = 'DUPLICADO_ARQUIVO'),
        COUNT(*) FILTER (WHERE classificacao = 'SEM_TELEFONE')
    INTO
        v_novos,
        v_existentes_telefone,
        v_cpf_telefone2,
        v_conflitos_telefone2,
        v_duplicados,
        v_sem_telefone
    FROM tmp_modelo_triagem;

    UPDATE modelo_estrela.stg_leads_site s
       SET processado = (t.classificacao <> 'NOVO')
      FROM tmp_modelo_triagem t
     WHERE s.ctid = t.row_ctid;

    IF v_novos > 0 THEN
        PERFORM * FROM modelo_estrela.sp_processar_stg_leads_site(p_upload_id);
    ELSE
        v_status := 'CONCLUIDO_SEM_NOVOS';
    END IF;

    BEGIN
        SELECT
            COALESCE(li.linhas_inseridas, 0),
            COALESCE(li.linhas_rejeitadas, 0),
            COALESCE(li.status, v_status)
        INTO v_inseridas, v_rejeitadas, v_status
        FROM modelo_estrela.logs_importacoes li
        WHERE li.upload_id = p_upload_id
        ORDER BY li.criado_em DESC
        LIMIT 1;
    EXCEPTION
        WHEN undefined_table OR undefined_column THEN
            v_inseridas := v_novos;
            v_rejeitadas := v_sem_telefone + v_conflitos_telefone2;
    END;

    v_rejeitadas := GREATEST(
        COALESCE(v_rejeitadas, 0),
        v_sem_telefone + v_conflitos_telefone2
    );

    v_mensagem := format(
        '%s recebida(s); %s nova(s); %s telefone(s) existente(s); %s telefone2 preenchido(s) por CPF; %s conflito(s) de telefone2; %s duplicada(s) no arquivo; %s sem telefone; %s inserida(s).',
        v_total, v_novos, v_existentes_telefone, v_cpf_telefone2,
        v_conflitos_telefone2, v_duplicados, v_sem_telefone, v_inseridas
    );

    RETURN QUERY
    SELECT
        p_upload_id,
        v_total,
        v_novos,
        v_existentes_telefone,
        v_cpf_telefone2 + v_conflitos_telefone2,
        v_duplicados,
        v_sem_telefone,
        v_novos,
        COALESCE(v_inseridas, 0),
        v_rejeitadas,
        COALESCE(v_status, 'CONCLUIDO'),
        v_mensagem;
END;
$function$;

-- Patch conservador da procedure principal da Anhanguera.
-- Faz a resolucao por celular e telefone2 e impede a troca destrutiva do celular principal.
DO $migration$
DECLARE
    v_definition text;
    v_original text;
BEGIN
    SELECT pg_get_functiondef(
        'modelo_estrela.sp_processar_stg_leads_site(text)'::regprocedure
    ) INTO v_definition;

    IF v_definition IS NULL THEN
        RAISE EXCEPTION 'modelo_estrela.sp_processar_stg_leads_site(text) nao encontrada';
    END IF;

    v_original := v_definition;

    v_definition := replace(
        v_definition,
        'modelo_estrela.fn_somente_numeros(p.celular) = t.celular_limpo',
        '(modelo_estrela.fn_somente_numeros(p.celular) = t.celular_limpo OR modelo_estrela.fn_somente_numeros(p.telefone2) = t.celular_limpo)'
    );

    v_definition := replace(
        v_definition,
        'modelo_estrela.fn_somente_numeros(celular) AS celular_limpo,',
        'telefone_limpo AS celular_limpo,'
    );

    v_definition := replace(
        v_definition,
        'FROM modelo_estrela.dim_pessoa\n        WHERE modelo_estrela.fn_celular_valido_basico(celular)',
        'FROM (SELECT sk_pessoa, celular AS telefone FROM modelo_estrela.dim_pessoa UNION ALL SELECT sk_pessoa, telefone2 AS telefone FROM modelo_estrela.dim_pessoa) telefones\n        WHERE modelo_estrela.fn_celular_valido_basico(telefone)'
    );

    v_definition := replace(
        v_definition,
        'modelo_estrela.fn_somente_numeros(celular)',
        'modelo_estrela.fn_somente_numeros(telefone)'
    );

    v_definition := replace(
        v_definition,
        'celular = CASE\n            WHEN modelo_estrela.fn_celular_valido_basico(t.celular_limpo)\n            THEN t.celular_limpo\n            ELSE p.celular\n        END,',
        'celular = CASE\n            WHEN NOT modelo_estrela.fn_celular_valido_basico(p.celular)\n             AND modelo_estrela.fn_celular_valido_basico(t.celular_limpo)\n            THEN t.celular_limpo\n            ELSE p.celular\n        END,\n        telefone2 = CASE\n            WHEN modelo_estrela.fn_celular_valido_basico(t.celular_limpo)\n             AND modelo_estrela.fn_somente_numeros(p.celular) IS DISTINCT FROM t.celular_limpo\n             AND NOT modelo_estrela.fn_celular_valido_basico(p.telefone2)\n            THEN t.celular_limpo\n            ELSE p.telefone2\n        END,'
    );

    IF v_definition = v_original THEN
        RAISE EXCEPTION 'Nenhum ponto conhecido foi encontrado para ajustar sp_processar_stg_leads_site';
    END IF;

    EXECUTE v_definition;
END;
$migration$;

-- UniFECAF: patch da funcao central para pesquisar nos dois telefones e
-- preservar o telefone principal ao receber novo numero para CPF existente.
DO $migration$
DECLARE
    v_definition text;
    v_original text;
BEGIN
    SELECT pg_get_functiondef(
        'unifecaf.sp_processar_upload(text,boolean)'::regprocedure
    ) INTO v_definition;

    IF v_definition IS NULL THEN
        RAISE EXCEPTION 'unifecaf.sp_processar_upload(text,boolean) nao encontrada';
    END IF;

    v_original := v_definition;

    v_definition := replace(
        v_definition,
        'unifecaf.fn_digits(p.celular)=t.cel_norm',
        '(unifecaf.fn_digits(p.celular)=t.cel_norm OR unifecaf.fn_digits(p.telefone2)=t.cel_norm)'
    );

    v_definition := replace(
        v_definition,
        'celular=COALESCE(NULLIF(trim(t.celular),''''),p.celular), email=',
        'celular=CASE WHEN unifecaf.fn_digits(p.celular) IS NULL THEN NULLIF(trim(t.celular),'''') ELSE p.celular END, telefone2=CASE WHEN t.cel_norm IS NOT NULL AND unifecaf.fn_digits(p.celular) IS DISTINCT FROM t.cel_norm AND unifecaf.fn_digits(p.telefone2) IS NULL THEN t.cel_norm ELSE p.telefone2 END, email='
    );

    IF v_definition = v_original THEN
        RAISE EXCEPTION 'Nenhum ponto conhecido foi encontrado para ajustar unifecaf.sp_processar_upload';
    END IF;

    EXECUTE v_definition;
END;
$migration$;

ANALYZE modelo_estrela.dim_pessoa;
ANALYZE unifecaf.dim_pessoa;

COMMIT;
