...
def _table_col_expr(view_name: str, alias_prefix: str, col: str, bq_type: str = "STRING", default_sql: str = "NULL") -> str:
    """Retorna uma expressão segura para usar fora do SELECT, como ORDER BY."""
    if _has_table_col(view_name, col):
        return f"SAFE_CAST({alias_prefix}.{col} AS {bq_type})"
    return f"CAST({default_sql} AS {bq_type})"


def _log_gestao_schema(view_name: str, operation_name: str, expected_cols: List[str]) -> None:
    """Log temporário de diagnóstico para comparar schema real x campos esperados."""
    try:
        columns = sorted(_table_columns(view_name))
        missing = [col for col in expected_cols if col not in columns]
        logger.info(
            "BQ %s schema view=%s columns=%s missing_expected=%s",
            operation_name,
            _table_id(view_name),
            columns,
            missing,
        ...
        logger.exception("BQ %s falhou ao ler schema view=%s", operation_name, _table_id(view_name))
        raise
...
def _run_gestao_query(sql: str, params: Optional[List[Any]] = None, operation_name: str = "gestão operacional") -> Any:
    ...
    logger.info("BQ %s iniciando sql=%s params=%s", operation_name, sql, params or [])
    job = client.query(sql, job_config=job_config, location=_bq_location())
    try:
        result = job.result(timeout=QUERY_TIMEOUT_SECONDS)
    except FuturesTimeoutError as exc:
        logger.exception("BQ %s timeout job_id=%s sql=%s params=%s", operation_name, getattr(job, "job_id", None), sql, params or [])
        job.cancel()
        raise TimeoutError(f"Consulta excedeu timeout de {QUERY_TIMEOUT_SECONDS}s") from exc
    except Exception:
        logger.exception("BQ %s falhou job_id=%s sql=%s params=%s", operation_name, getattr(job, "job_id", None), sql, params or [])
        raise
...
def query_gestao_operacao() -> Dict[str, Any]:
    ...
    expected_cols = [
        "total_leads", "aguardando_primeiro_contato", "sem_acionamento", "matriculados",
        "leads_parados", "leads_criticos", "leads_alta_prioridade",
        "media_horas_primeiro_contato", "score_medio_operacao",
    ]
    _log_gestao_schema(view_name, "query_gestao_operacao", expected_cols)
    select_cols = ",\n      ".join([
...
def query_gestao_produtividade() -> List[Dict[str, Any]]:
    ...
    expected_cols = [
        "consultor_comercial", "total_leads", "leads_nao_disparados", "leads_disparados",
        "matriculados", "taxa_matricula_pct", "ultima_atividade", "score_medio_carteira",
        "leads_sem_movimento_7_dias", "leads_criticos", "leads_alta_prioridade",
    ]
    _log_gestao_schema(view_name, "query_gestao_produtividade", expected_cols)
    matriculados_order_expr = _table_col_expr(view_name, "p", "matriculados", bq_type="INT64", default_sql="0")
    taxa_order_expr = _table_col_expr(view_name, "p", "taxa_matricula_pct", bq_type="FLOAT64", default_sql="0")
    select_cols = ",\n      ".join([
    ...
    ORDER BY COALESCE({matriculados_order_expr}, 0) DESC, COALESCE({taxa_order_expr}, 0) DESC
    """
...
def query_gestao_fila_operacional(limit: int = GESTAO_FILA_LIMIT) -> List[Dict[str, Any]]:
    ...
    expected_cols = [
        "nome", "celular", "curso", "polo", "status", "consultor_comercial",
        "score_prioridade", "nivel_prioridade", "etapa_operacional", "dias_sem_acao",
        "nunca_disparado", "horas_ate_primeiro_contato", "flag_matriculado",
        "data_inscricao", "data_ultima_acao",
    ]
    _log_gestao_schema(view_name, "query_gestao_fila_operacional", expected_cols)
    ...
    score_order_expr = _table_col_expr(view_name, "lp", "score_prioridade", bq_type="FLOAT64", default_sql="0")
    select_cols = ",\n      ".join([
    ...
    ORDER BY COALESCE({score_order_expr}, 0) DESC
    LIMIT @limit
    """
