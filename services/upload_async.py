...
def _progress_row(upload_id: str) -> Dict[str, Any]:
    for institution in ("anhanguera", "unifecaf"):
        ...
        try:
            ...
            if rows:
                row = rows[0]; row["institution"] = institution; return row
        except Exception:
...
def _set_progress(cfg: Dict[str, str], upload_id: str, status: str, etapa: str, progresso: float, **metrics: Any) -> None:
    params = {"upload_id": upload_id, "status": status, "etapa": etapa, "progresso": progresso, "linhas_processadas": metrics.get("linhas_processadas"), "linhas_inseridas": metrics.get("linhas_inseridas"), "linhas_ignoradas": metrics.get("linhas_ignoradas"), "linhas_rejeitadas": metrics.get("linhas_rejeitadas"), "mensagem": metrics.get("mensagem"), "erro": metrics.get("erro")}
...
def _copy_dataframe_to_staging(cfg: Dict[str, str], prepared, upload_id: str, mode: str, routine_name: str, filename: str) -> None:
    engine = db.get_engine(); raw = engine.raw_connection(); cursor = None
    try:
        cursor = raw.cursor(); cursor.execute("SET LOCAL lock_timeout = '5s'"); cursor.execute("SET LOCAL statement_timeout = '180s'")
        ...
        cursor.execute(f"INSERT INTO {cfg['schema_ident']}.{cfg['progress']} (upload_id, modo, rotina, arquivo, status, etapa, linhas_total, progresso) VALUES (%s,%s,%s,%s,'STAGING','GRAVANDO_STAGING',%s,10)", (upload_id, mode, routine_name, filename, len(prepared)))
        if cfg["institution"] == "unifecaf":
            cursor.execute(f"INSERT INTO {cfg['schema_ident']}.logs_importacoes (upload_id,nome_arquivo,status,etapa,total_linhas,linhas_recebidas) VALUES (%s,%s,'RECEBIDO','STAGING',%s,%s) ON CONFLICT (upload_id) DO NOTHING", (upload_id, filename, len(prepared), len(prepared)))
        columns = list(prepared.columns); buffer = io.StringIO()
        ...
        buffer.seek(0); column_sql = ",".join(db._safe_ident(column) for column in columns)
        ...
        if hasattr(cursor, "copy_expert"): cursor.copy_expert(copy_sql, buffer)
            ...
            with cursor.copy(copy_sql) as copy:
                while True:
                    ...
                    if not chunk: break
                    copy.write(chunk)
        else: raise RuntimeError("O driver PostgreSQL não oferece suporte a COPY FROM STDIN.")
...
def enqueue_upload_dataframe(df, filename: str, mode: str, routine_name: str, institution: str = "anhanguera") -> Dict[str, Any]:
    cfg = _config(institution); upload_id = uuid.uuid4().hex; started = time.monotonic()
    prepared = db._prepare_upload_dataframe(df, filename, upload_id)
    if cfg["institution"] == "unifecaf": prepared = _prepare_for_unifecaf(prepared)
    total_rows = len(prepared)
    if total_rows <= 0: raise ValueError("A planilha não possui linhas para importar.")
    stg_cols = STAGING_COLUMNS[cfg["institution"]]; prepared = prepared[[c for c in prepared.columns if c in stg_cols]].copy()
    if "upload_id" not in prepared.columns: prepared["upload_id"] = upload_id
