...
ACTIVE_STATUS_EXCLUDED = ("CONCLUIDO", "MAT", "CANCELADO")
...
def create_operational_tables() -> Dict[str, Any]:
    ...
    return {"created": ["op_lotes_disparo", "op_lote_leads", "op_lead_eventos", "op_bigquery_sync"]}


def get_dashboard() -> Tuple[Dict[str, Any], bool]:
    data = _single(f"""
    SELECT
      (SELECT COUNT(*) FROM {_ref(VIEW_LEADS_PRIORIZADOS)} l LEFT JOIN {_ref('op_lote_leads')} op ON l.sk_pessoa=op.sk_pessoa AND op.status_atendimento NOT IN ('CONCLUIDO','MAT','CANCELADO') WHERE op.sk_pessoa IS NULL AND COALESCE(l.flag_matriculado,FALSE)=FALSE) AS leads_disponiveis,
      (SELECT COUNT(*) FROM {_ref('op_lote_leads')} WHERE status_atendimento NOT IN ('CONCLUIDO','MAT','CANCELADO')) AS leads_em_lote,
...
def get_leads_disponiveis(filters: Mapping[str, Any], meta: Mapping[str, Any]) -> Tuple[Dict[str, Any], bool]:
    ...
    LEFT JOIN {_ref('op_lote_leads')} op
     ...
     AND op.status_atendimento NOT IN ('CONCLUIDO', 'MAT', 'CANCELADO')
    WHERE {' AND '.join(where)}
    ORDER BY l.score_prioridade DESC, l.data_inscricao ASC
...
def criar_lote(payload: Mapping[str, Any]) -> Tuple[Dict[str, Any], bool]:
    ...
    for lead in leads:
        ...
        WHERE NOT EXISTS (SELECT 1 FROM {_ref('op_lote_leads')} WHERE sk_pessoa=@sk_pessoa AND status_atendimento NOT IN ('CONCLUIDO','MAT','CANCELADO'))""", params, "operacional_criar_lote_lead")
        _evento(lote_id, lead.get("sk_pessoa"), lead.get("cpf"), "CRIACAO_LOTE", None, "PENDENTE", "Lead incluído no lote", payload.get("criado_por"))
    ...
    return {"lote_id": lote_id, "quantidade_leads": len(leads), "status_lote": "ABERTO", "aviso": aviso}, False
...
def start_lote(lote_id: str) -> Tuple[Dict[str, Any], bool]:
    ...
    _evento(lote_id, None, None, "INICIO_LOTE", None, "EM_ANDAMENTO", "Lote iniciado", None)
    invalidate_gestao_cache()
...
def finish_lote(lote_id: str) -> Tuple[Dict[str, Any], bool]:
    p = [bigquery.ScalarQueryParameter("lote_id", "STRING", lote_id)]
    ...
    _evento(lote_id, None, None, "FINALIZACAO_LOTE", None, "CONCLUIDO", "Lote finalizado", None)
    invalidate_gestao_cache()
...
def update_lead_status(sk_pessoa: int, payload: Mapping[str, Any]) -> Tuple[Dict[str, Any], bool]:
    ...
    _evento(lote_id, sk_pessoa, old.get("cpf"), "STATUS_LEAD", old.get("status_atendimento"), status, payload.get("observacao") or "Status atualizado", payload.get("usuario"))
    invalidate_gestao_cache()
